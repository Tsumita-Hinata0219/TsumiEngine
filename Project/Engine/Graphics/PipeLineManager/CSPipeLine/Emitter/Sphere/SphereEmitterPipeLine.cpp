...
void SphereEmitterPipeLine::SetUpRootSignature(D3D12_ROOT_SIGNATURE_DESC& descriptionRootSignature)
{
	...
	particleRange[0].BaseShaderRegister = 0; // レジスター番号
	particleRange[0].NumDescriptors = 1; // 数は1つ
	particleRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
	particleRange[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; // offsetを自動計算

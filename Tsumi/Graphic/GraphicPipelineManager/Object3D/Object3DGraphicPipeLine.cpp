...
// 初期化処理
void Object3DGraphicPipeLine::Initialize() {}


// PSOを構築する
void Object3DGraphicPipeLine::SetupObject3DPso()
{
	Object3DGraphicPipeLine* object3dGpl = Object3DGraphicPipeLine::GetInstance();


	/* --- RootSignatureを作成 --- */
	D3D12_ROOT_SIGNATURE_DESC descriptionRootSignature{};
	object3dGpl->SetupRootSignature(descriptionRootSignature);
	...
	object3dGpl->SetupBlendState(pBlendDesc, BlendNone);


	/* --- RasiterzerStateを設定する --- */
	D3D12_RASTERIZER_DESC rasterizerDesc{};
	object3dGpl->SetupRasterizerState(rasterizerDesc);
	...
	object3dGpl->CompileShaders(vertexShaderBlob, pixelShaderBlob);


	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateDesc{};

	graphicsPipelineStateDesc.pRootSignature = Object3DGraphicPipeLine::GetInstance()->object3dPso_.rootSignature; // RootSignature
	...
	hr = DirectXCommon::GetInstance()->GetDevice()->CreateGraphicsPipelineState(
		...
		IID_PPV_ARGS(&Object3DGraphicPipeLine::GetInstance()->object3dPso_.graphicsPipelineState));
	assert(SUCCEEDED(hr));
}


// RootSignatureのセットアップ
void Object3DGraphicPipeLine::SetupRootSignature(D3D12_ROOT_SIGNATURE_DESC& descriptionRootSignature)
{
	...
	HRESULT hr_ = D3D12SerializeRootSignature(
		...
		&Object3DGraphicPipeLine::GetInstance()->object3dPso_.signatureBlob,
		&Object3DGraphicPipeLine::GetInstance()->object3dPso_.errorBlob);
	if (FAILED(hr_)) {
		Log(reinterpret_cast<char*>(Object3DGraphicPipeLine::GetInstance()->object3dPso_.errorBlob->GetBufferPointer()));
		assert(false);
	}
	...
	hr_ = DirectXCommon::GetInstance()->GetDevice()->CreateRootSignature(
		0,
		Object3DGraphicPipeLine::GetInstance()->object3dPso_.signatureBlob->GetBufferPointer(),
		Object3DGraphicPipeLine::GetInstance()->object3dPso_.signatureBlob->GetBufferSize(),
		IID_PPV_ARGS(&Object3DGraphicPipeLine::GetInstance()->object3dPso_.rootSignature));
	assert(SUCCEEDED(hr_));
}


// InputLayoutのセットアップ
void Object3DGraphicPipeLine::SetupInputElementDescs(D3D12_INPUT_ELEMENT_DESC& inputElementDescs, LPCSTR SemanticName, UINT SemanticIndex, DXGI_FORMAT Format, UINT AlignedByteOffset)
{
...
void Object3DGraphicPipeLine::SetupInputLayout(D3D12_INPUT_LAYOUT_DESC& inputLayoutDesc, const D3D12_INPUT_ELEMENT_DESC* inputElementDescs, UINT numInputElements)
{
...
// BlendStateのセットアップ
void Object3DGraphicPipeLine::SetupBlendState(D3D12_RENDER_TARGET_BLEND_DESC& blendDesc, BlendMode blendMode)
{
...
void Object3DGraphicPipeLine::SetupRasterizerState(D3D12_RASTERIZER_DESC& rasterizerDesc)
{
	// 裏面(時計回り)を表示しない
...
{
	vertexShaderBlob = ShaderManager::GetInstance()->GetModelShader("Object3D").VertexBlob;
	assert(vertexShaderBlob != nullptr);

	pixelShaderBlob = ShaderManager::GetInstance()->GetModelShader("Object3D").PixelBlob;

...
/// PSOを構築する
/// </summary>
PsoProperty DecalPipeLine::SetUpPso()
{
	/* --- RootSignatureを作成 --- */
	...
	/* --- InputLayoutを設定する --- */
	std::array<D3D12_INPUT_ELEMENT_DESC, 1> inputElementDesc = {
	...
	SetUpModelShader(vertexShaderBlob, pixelShaderBlob, "Decal");
...
/// </summary>
void DecalPipeLine::SetUpRootSignature(D3D12_ROOT_SIGNATURE_DESC& descriptionRootSignature)
{
	...
	materiaTexture[0].BaseShaderRegister = 1; // レジスタ番号
	...
	rootParameters[4].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL; // PixShaderで使う
	rootParameters[4].DescriptorTable.pDescriptorRanges = materiaTexture; // Tableの中身の配列を指定
	rootParameters[4].DescriptorTable.NumDescriptorRanges = _countof(materiaTexture); // Tableで利用する
